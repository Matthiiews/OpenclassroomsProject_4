...
class DatabaseController:
    """ Define a class Database Controller """
    def __init__(self):
...
if __name__ == "__main__":
    # Création d'une instance de DatabaseController
    db_controller = DatabaseController()

    # Test de la fonction load_tournament_db()
    tournaments = db_controller.load_tournament_db()
    print("Tournaments:")
    for tournament in tournaments:
        print(tournament)

    # Test de la fonction save_tournament_db()
    new_tournament = Tournament()  
    db_controller.save_tournament_db(new_tournament)

    # Test de la fonction update_tournament_db()
    tournament_id = 1  
    rounds = []  
    players = []  
    round_number = 1  
    db_controller.update_tournament_db(tournament_id, rounds, players, round_number)

    # Test de la fonction update_timer()
    tournament_id = 1  
    timer = "2023-06-12 12:00:00"  
    info = "start"  
    db_controller.update_timer(tournament_id, timer, info)

    # Test de la fonction load_player_db()
    players = db_controller.load_player_db()
    print("Players:")
    for player in players:
        print(player)

    # Test de la fonction save_player_db()
    new_player = Player()  
    db_controller.save_player_db(new_player)

    # Test de la fonction update_player_db()
    player = {}  
    info = "John Doe"  
    option = "name"  
    db_controller.update_player_db(player, info, option)
"""